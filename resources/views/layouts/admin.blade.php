    ...
    <div class="min-h-full lg:flex">
        <aside class="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 bg-slate-950 text-slate-300 border-r border-slate-800/80">
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                ...
                <a href="{{ route('dashboard.site-footer.edit') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition {{ request()->routeIs('dashboard.site-footer.*') ? 'bg-white/10 text-white border border-white/10' : 'hover:bg-white/5 hover:text-white' }}">
                    <svg class="h-5 w-5 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                    {{ __('Site footer') }}
                </a>
            </nav>
        ...
        <div class="flex-1 lg:pl-64 flex flex-col min-h-screen min-w-0">
            <header class="sticky top-0 z-40 flex min-h-16 shrink-0 items-center gap-3 border-b border-slate-200/80 bg-white/80 px-4 pt-[env(safe-area-inset-top)] backdrop-blur-md sm:gap-4 lg:px-8">
                <details class="relative lg:hidden group">
                    ...
                    <div class="absolute left-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50 py-2">
                        ...
                        <a href="{{ route('dashboard.site-footer.edit') }}" class="block px-4 py-2 text-sm hover:bg-slate-50">{{ __('Site footer') }}</a>
            ...
            <main class="flex-1 min-w-0 overflow-x-hidden p-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] sm:p-6 lg:p-10">
                @if (session('success'))
